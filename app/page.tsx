...
const C = {
  ...
  headline:  'var(--font-headline)'  as const,  // DM Sans — editorial
...
// Section headline — uses DM Sans (editorial voice)
const H2 = (overrides: React.CSSProperties = {}): React.CSSProperties => ({
...
// ─── Static data ───────────────────────────────────────────────────────────────
const STATS = [
  { n: '< 10 min', l: 'sign-up to active swarm' },
  { n: '< 2 hrs',  l: 'first enriched output' },
  { n: '60–75%',   l: 'token cost reduction (month 3)' },
  { n: 'N=∞',      l: 'threat intel vs. self-hosted N=1' },
];
...
const CRISES = [
  {
    ...
    body: 'Two agents in a shared workspace with no structural coordination constraints default to open-ended peer-to-peer messaging. Token spend hits $200–$3,000 before a blunt manual spending cap fires — which kills the agent, not the loop.',
    fix: 'Hierarchical FSM + SQLite task list. Every agent interaction is a discrete, locked, auditable task. Horizontal messaging is structurally impossible. Loops cannot form.',
  },
  {
    n: '02',
    title: 'Amnesiac background jobs',
    body: "Hermes Agent's cron scheduler hardcodes skip_memory=True to prevent background prompts from contaminating foreground session history. An agent that learns something at 2 AM forgets it completely before the 8 AM run.",
    fix: 'CADVP daemon. Background agents write to an isolated JSONL event stream on disk. A host-layer monitor intercepts, validates, and commits verified outputs to GBrain under the agent\'s identity.',
  },
  {
    n: '03',
    title: 'Credential leakage via injection',
    body: 'Agents holding raw API keys in their active context window are trivially exploitable. A malicious string in any content they process — a Slack message, a GitHub issue, a crawled webpage — can exfiltrate credentials to an attacker endpoint.',
    fix: 'OneCLI Secret Vault Proxy. Credentials live in a host-layer vault and are never in agent scope. Agents hold placeholders; the proxy injects real credentials at the network layer. A fully-compromised agent has nothing to exfiltrate.',
  },
];
...
const DEPARTMENTS = [
  ...
  },
  {
    ...
    desc: 'Turns a ticket into a structured spec, executes approved steps in an isolated sandbox, runs tests, and returns a consolidated PR.',
...
const TIERS = [
  ...
  { name: 'Professional', price: '$149', note: '/mo + LLM',          badge: 'Most popular',detail: 'Fleet intelligence makes your agents smarter from collective data.',   features: ['Fleet intelligence', 'Threat feed < 60 min', '2 departments', 'Weekly Dream-Cycle'] },
  { name: 'Business',  price: '$399',  note: '/mo + LLM',            badge: null,          detail: 'SOC 2 inheritance at a fraction of a $50K+ independent audit.',       features: ['SOC 2 Type II', 'SSO / SCIM', '3 departments', 'Nightly optimization'] },
  { name: 'Agency',    price: '$249',  note: '/mo + $39/client',     badge: null,          detail: 'Cross-client skill transfer. Pay based on value delivered.',          features: ['Multi-workspace', 'Cross-client skills', 'Agent federation', 'Unlimited staff seats'] },
  { name: 'Enterprise', price: '$799', note: '/mo + LLM',            badge: null,          detail: 'Dedicated single-tenant TEE. HIPAA BAA. Custom SLA with financial credits.', features: ['Dedicated TEE', 'HIPAA BAA', '99.5% SLA credits', 'Private skill marketplace'] },
];
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 1 }}>
          {CRISES.map((c) => (
            ...
            >
              ...
              <div style={{ fontFamily: C.headline, fontSize: '1.3rem', fontWeight: 700, color: C.primary, marginBottom: 14, letterSpacing: '-0.025em', lineHeight: 1.2 }}>
                {c.title}
              </div>
              <p style={{ fontFamily: C.body, fontSize: 14, color: C.secondary, lineHeight: 1.72, marginBottom: 24 }}>
                {c.body}
              </p>
              <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 20, fontFamily: C.mono, fontSize: 11, color: '#6D6D78', lineHeight: 1.6 }}>
                <span style={{ color: C.primary, marginRight: 8 }}>FIX ›</span>
                {c.fix}
              </div>
            </div>
          ))}
...
function DepartmentsSection() {
  ...
  return (
    <section style={{ ...sep, padding: '96px 0' }}>
      <div style={wrap} ref={ref}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 20, flexWrap: 'wrap', marginBottom: 48 }}>
          ...
          <Link
            href="/departments"
            data-reveal
            style={{ fontFamily: C.mono, fontSize: 10, color: C.secondary, letterSpacing: '0.12em', whiteSpace: 'nowrap' }}
          ...
          {DEPARTMENTS.map((dept) => (
            ...
            >
              ...
              <div style={{ padding: '24px 24px 16px' }}>
                <div style={{ fontFamily: C.mono, fontSize: 9, color: C.secondary, letterSpacing: '0.18em', textTransform: 'uppercase', marginBottom: 12, border: `1px solid ${C.border}`, display: 'inline-block', padding: '3px 8px', borderRadius: 4 }}>
                  {dept.tag}
                </div>
                <h3 style={{ fontFamily: C.headline, fontSize: '1.25rem', fontWeight: 700, letterSpacing: '-0.025em', color: C.primary, margin: '0 0 10px' }}>
                  {dept.name}
                </h3>
                <p style={{ fontFamily: C.body, fontSize: 13.5, color: C.secondary, lineHeight: 1.62, margin: '0 0 14px' }}>
                  {dept.desc}
                </p>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {dept.roles.map((r) => (
                    <span key={r} style={{ fontFamily: C.mono, fontSize: 9, color: '#6D6D78', background: C.raised, border: `1px solid ${C.border}`, borderRadius: 4, padding: '3px 8px', letterSpacing: '0.06em' }}>
                      {r}
                    </span>
                  ))}
                </div>
              ...
              <div style={{ margin: '0 12px 12px', borderRadius: 10, background: '#16181C', border: '1px solid #222428', padding: '12px 14px', flex: 1 }}>
                {dept.preview.map((m, i) => (
                  <div key={i} style={{ display: 'flex', gap: 8, marginBottom: i < dept.preview.length - 1 ? 8 : 0 }}>
                    <div style={{ width: 18, height: 18, borderRadius: 4, background: m.a ? '#242428' : '#2A2A2F', border: `1px solid ${m.a ? '#38383E' : '#2A2A2F'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: C.mono, fontSize: 7, color: m.a ? '#A09D98' : '#6D6D78', flexShrink: 0, marginTop: 1 }}>
                      {m.a ? '✦' : 'U'}
                    </div>
              ...
              {/* Hover reveal — stat + detail (Moranta hover layer) */}
              <div className="dept-hover-reveal" style={{ padding: '0 24px 20px', borderTop: `1px solid ${C.border}`, paddingTop: 16 }}>
                <div style={{ fontFamily: C.mono, fontSize: 10, color: C.primary, letterSpacing: '0.08em', marginBottom: 4 }}>
                  ↗ {dept.stat}
                </div>
              ...
              {/* Static stat line (visible when not hovering) */}
              <div style={{ padding: '10px 24px 18px', fontFamily: C.mono, fontSize: 10, color: '#46464E', letterSpacing: '0.08em', transition: 'opacity 0.2s ease' }} className="dept-stat-static">
                ↗ {dept.stat}
              </div>
            </article>
          ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 12, marginTop: 12 }}>
          {[['Marketing Content', 'Q3 2026'], ['Customer Success', 'Q4 2026']].map(([name, when]) => (
            <div key={name} data-reveal style={{ border: `1px dashed ${C.border}`, borderRadius: 14, padding: '18px 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              <span style={{ fontFamily: C.headline, fontSize: '1rem', color: C.secondary, letterSpacing: '-0.01em' }}>{name}</span>
      ...
      {/* Hide the static stat line on hover via CSS */}
      <style>{`
        .dept-card:hover .dept-stat-static { opacity: 0; }
        .dept-card:hover .dept-hover-reveal { display: block; }
      `}</style>
    </section>
  );
}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '6vw', alignItems: 'start' }}>
          <div>
            ...
            <p data-reveal style={{ fontFamily: C.body, fontSize: '1rem', lineHeight: 1.72, color: C.secondary, marginBottom: 32 }}>
              Sockt charges only for orchestration infrastructure — FSM coordination, GBrain memory, TEE isolation, and fleet intelligence. Your inference bill stays between you and your provider, at their rates, with zero markup.
            </p>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              {[
                ...
                { layer: 'Egress Allowlist', body: 'All outbound traffic routes through per-customer domain rules. DNS covert channels blocked. Injection attempts logged and distributed to fleet threat feed in < 60 min.' },
                { layer: 'HITL Gates', body: 'Tier 1 auto-executes. Tier 2 needs human approval before any external action. Tier 3 is permanently blocked at the proxy layer — overrides by agent reasoning are architecturally impossible.' },
              ].map((item) => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '6vw', alignItems: 'center' }}>
          <div>
            ...
            <p data-reveal style={{ fontFamily: C.body, fontSize: '1rem', lineHeight: 1.72, color: C.secondary }}>
              A single self-hosted deployment has N=1 data. No baseline, no collective threat intelligence, no proactive API monitoring. Paid tiers connect to a network that becomes more valuable with every deployment added.
            </p>
          </div>
          <div data-reveal style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1 }}>
            {[
              ...
              <div key={item.l} style={{ border: `1px solid ${C.border}`, padding: '24px 20px', background: C.surface }}>
                <div style={{ fontFamily: C.headline, fontSize: 'clamp(1.1rem, 3vw, 1.8rem)', fontWeight: 700, letterSpacing: '-0.03em', color: C.primary, lineHeight: 1, marginBottom: 10 }}>
                  {item.n}
                </div>
          ...
          {TIERS.map((tier) => (
            ...
            >
              ...
              <div>
                ...
                <div style={{ fontFamily: C.headline, fontSize: 'clamp(1.5rem, 3vw, 2rem)', fontWeight: 700, letterSpacing: '-0.03em', color: C.primary, lineHeight: 1, marginBottom: 4 }}>
                  {tier.price}
                </div>
              ...
              {/* Hover reveal — one-line detail */}
              <div className="price-hover-detail" style={{ fontFamily: C.body, fontSize: 12, color: C.secondary, lineHeight: 1.55, marginTop: -4 }}>
                {tier.detail}
              </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '6vw', alignItems: 'start' }}>
          ...
          <div>
            <div data-reveal style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              {OSS_MODULES.map((m) => (
                <div key={m.name} style={{ display: 'flex', border: `1px solid ${C.border}`, background: C.surface }}>
                  <div style={{ fontFamily: C.mono, fontSize: 10, color: C.primary, letterSpacing: '0.06em', padding: '14px 16px', borderRight: `1px solid ${C.border}`, minWidth: 104, flexShrink: 0, display: 'flex', alignItems: 'center' }}>
                    {m.name}
                  </div>
...
export default function Home() {
  ...
  useReveal(howItWorksRef, { selector: '[data-reveal]', stagger: 0.1, y: 32, duration: 0.88 });
  ...
  return (
    <>
      {/* First-visit loader — unobtrusive, < 1.4s, session-persisted */}
      <PageLoader />
      <Nav />

      <main style={{ color: C.primary, background: C.void, overflowX: 'hidden' }}>

        {/* ── HERO — fast / clear / un-precious (Moranta spec: NO slow editorial pacing here) ── */}
        <section
          style={{
            position: 'relative',
            minHeight: '92vh',
        ...
        >
          <div style={{ ...wrap, width: '100%' }}>
            ...
            >
              {/* Left — direct, not lingered-over */}
              <div>
                <span style={{ ...label(), marginBottom: 24 }}>AI Workforce Platform</span>
                <h1
                  style={{
                    fontFamily: C.headline,       /* DM Sans — editorial voice */
              ...
              {/* Right — Slack mock with reveal animation */}
        ...
        {/* ── STATS STRIP (no reveal — must be immediate after hero) ── */}
        <div style={{ borderTop: `1px solid ${C.border}`, borderBottom: `1px solid ${C.border}`, background: C.surface }}>
          <div style={wrap}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 0 }}>
              {STATS.map((s, i) => (
                <div key={s.l} style={{ padding: '28px 24px', borderRight: i < STATS.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                  <div style={{ fontFamily: C.headline, fontSize: 'clamp(1.4rem, 3vw, 2.2rem)', fontWeight: 700, letterSpacing: '-0.03em', color: C.primary, lineHeight: 1, marginBottom: 8 }}>
                    {s.n}
                  </div>
        ...
        {/* ── PROOF SECTIONS — Moranta "emerge from darkness" treatment ── */}
        <CrisesSection />

        {/* ── HOW IT WORKS ── */}
        <section style={{ ...sep, padding: '96px 0', background: C.surface }}>
          <div style={wrap} ref={howItWorksRef}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '6vw', alignItems: 'start' }}>
                    ...
                    <div key={title} data-reveal>
                      <div style={{ display: 'flex', gap: 0 }}>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 32, flexShrink: 0 }}>
                          <div style={{ width: 24, height: 24, borderRadius: '50%', border: `1px solid ${C.border}`, background: C.raised, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: C.mono, fontSize: 9, color: C.secondary, flexShrink: 0 }}>
              ...
              <div>
                ...
                <div style={{ marginTop: 28, display: 'flex', flexDirection: 'column', gap: 1 }}>
                  {[
                    { layer: 'Execution', body: 'Hermes agents run Plan→Act→Observe→Reflect inside AMD SEV-SNP / Intel SGX enclaves. Powered entirely by your own LLM key — no inference markup.' },
                    { layer: 'Coordination', body: 'OpenClaw gateway + FSM task list. No horizontal agent messaging. Every interaction is a discrete, locked, logged task record with a defined terminal state.' },
        ...
        {/* ── FINAL CTA — fast / clear / un-precious (same rule as hero) ── */}
        <section style={{ ...sep, padding: '128px 0', background: C.surface, textAlign: 'center' }}>
          <div style={wrap}>
